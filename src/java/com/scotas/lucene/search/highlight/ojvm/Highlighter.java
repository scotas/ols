package com.scotas.lucene.search.highlight.ojvm;
  ...
  static public BigDecimal ODCITableFetch(BigDecimal ctx, BigDecimal nrows,
    ...
    if (rset.next()) {
      ...
      for (int i = 0; i < numCols; i++) {
        ...
        logger.info("fetch '"+cols[i]+"' val: '"+val+"' class: "+((val != null) ?  val.getClass() : "val is null ") + " is highlighted: " + highlightedCols[i]);
        if (val instanceof String && highlightedCols[i]) {
          ...
          // supported highlight columns
          CLOB hval = CLOB.createTemporary(conn, true, CLOB.DURATION_SESSION);
          if (((CLOB)val).getLength() > 0) {
              try {
                String text = DefaultUserDataStore.readStream(new BufferedReader(((CLOB)val).characterStreamValue()));
                text =
                  highlighter.getBestFragments(analyzer.tokenStream(sctx.getColumnName(),
